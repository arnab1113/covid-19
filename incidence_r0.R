...
incidence<-function(input,district,dty){
  ...
  newcases2<-function(i){
    rowid<-seq(1:n)[input$Date.Announced==dates[i] & input$Detected.District==district & input$Current.Status=="Hospitalized"]
...
estR=estimate_R(incid[incid>0],method="parametric_si",
